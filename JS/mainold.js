...
const { SettingsManagerMessenger } = require("./SettingsManager.js");
...
function send_settings(to_window) {
	if (to_window === "main") {
		if (main_window) {
			...
			SMMessenger = new SettingsManagerMessenger();
			main_window.webContents.send("settings-messenger", SMMessenger);
			console.log(`Dummy variable: ${SMMessenger.dummy_variable}`);
		}
