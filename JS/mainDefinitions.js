...
// Possible modes for acquiring VMI images
// NOTE: Change these to upper case when implementation changes are complete
const Scanning_Mode = {
	SEVI: "sevi",
	IRSEVI: "ir-sevi",
};

// Possible detachment laser setups
const Detachment_Mode = {
	STANDARD: "STANDARD",
	DOUBLED: "DOUBLED",
	RAMAN: "RAMAN",
	IRDFG: "IR-DFG",
...
// Possible IR Excitation laser setups
const Excitation_Mode = {
	NIR: "NIR", // Near IR
	IIR: "IIR", // Intermediate IR
	MIR: "MIR", // Mid IR
	FIR: "FIR", // Far IR
};

// Possible VMI settings
const VMI_Mode = {
	V1: "V1",
	V2: "V2",
	V3: "V3",
...
// Settings for auto-stopping VMI images
const Autostop = {
	NONE: "NONE",
	ELECTRONS: "ELECTRONS",
	FRAMES: "FRAMES",
...
// IPC messages that can be sent/received from the main process
// NOTE: Before seriously using these, they should be changed to be the same for all windows
//	then you could have the enums stored in a separate file that all windows access
//	(would need to get the main process to use it too)
const IPC_Messages = {
	SETTINGS_INFO: "settings-information",
	CLOSE_CAMERA: "close-camera",
	READY: "main-window-ready",
	LOADED: "main-window-loaded",
	UPDATE_SAVE_DIR: "update-save-directory",
	UPDATE_SETTINGS: "update-settings",
	SCAN_UPDATE: "scan-update",
	NEW_FRAME: "new-camera-frame",
	USE_HYBRID: "hybrid-method",
};
...
// I think maybe I should make an overall Spectrum class, then make subclasses for
//	SEVISpectrum and IRSpectrum

// Is it better to use a dict for spectra that uses the Spectrum id as the key? That
//	way you don't have to loop over everything or worry about the order

// Need to plan out what the Spectrum classes are going to do. If they get too complicated,
//	should split up into smaller classes

// scan object should def get split up into classes like this. Previous scans should be similar to
//	what I'm doing with Spectrum. Single shot can also be a simple class.

// Should add list of checkboxes next to PES display to select with Spectra to display
//	and add radio option to display R or eBE as x axis (and to normalize?)

//
// 	- Separate creation from use
//	- Single responsibility use
...
class Spectrum {
	...
	get file_name() {
		return `${this.formatted_date}i${this.id_str}.pes`;
	}
	...
	get_data(use_ebe = true, normalize = true) {
		if (use_ebe) {
			console.log("Need to add R->eBE conversion");
		} else {
			if (normalize) {
				let max_intensity = Math.max(this.data.intensity);
				let norm_intensity = this.data.intensity.map((i) => i / max_intensity);
				let norm_anisotropy = this.data.anisotropy.map((i) => i / max_intensity);
				return [this.data.radial_values, norm_intensity, norm_anisotropy];
			} else {
				return [this.data.radial_values, this.data.intensity, this.data.anisotropy];
			}
		}
	}
	...
	static new(image_id, scanning_mode) {
		if (scanning_mode === Scanning_Mode.SEVI) {
			return new SEVISpectrum(image_id);
		} else if (scanning_mode === Scanning_Mode.IRSEVI) {
			return new IRSpectrum(image_id);
		} else {
			throw Error(`'${scanning_mode}' is not a valid scanning mode type`);
		}
	}
}

class SEVISpectrum extends Spectrum {
	constructor(image_id) {
		super(image_id); // Evaluate constructor() of parent Spectrum class
		this.scanning_mode = Scanning_Mode.SEVI;
	}
}

// This class should encapsulate both IR On and IR Off to make things easy
class IRSpectrum extends Spectrum {
	constructor(image_id) {
		super(image_id); // Evaluate constructor() of parent Spectrum class
		this.scanning_mode = Scanning_Mode.IRSEVI;
	}

	get ir_file_name() {
		return `${this.formatted_date}i${this.id_str}_IR.pes`;
	}

	add_data(data) {
		this.data = {
			radial_values: data.radial_values,
			ir_off_intensity: data.ir_off_intensity,
			ir_on_intensity: data.ir_on_intensity,
			ir_off_anisotropy: data.ir_off_anisotropy,
			ir_on_anisotropy: data.ir_on_anisotropy,
		};
	}
}

const Spectra = {
	all: {}, // Spectra are stored in a dict with the image_id as the key, that way order doesn't matter
	current: undefined,
	latest: undefined,
	...
	update_latest: (spectrum) => {
		Spectra.latest = spectrum;
	},
	clear_latest: () => {
...
// Setting this up similar to Spectrum
// Should keep track of
//	- Image id, file names
//	- Scan type, electron count, frame count
//	- Number of frames/electrons until saved
//		(Everything in electrons.total)
//	- IR on/off counts should be stored by both SEVI and IRImage

// Should there be a blank Image class stored in current instead of undefined?
//	that way there doesn't need to be any logic done about whether to add electrons to current image
...
class Image {
	constructor(image_id) {
		...
		this.electron_count_off = 0;
		this.electron_count_on = 0;
		this.frames_off = 0;
		this.frames_on = 0;
	}
	...
	static new(image_id, scanning_mode) {
		if (scanning_mode === Scanning_Mode.SEVI) {
			return new SEVIImage(image_id);
		} else if (scanning_mode === Scanning_Mode.IRSEVI) {
			return new IRImage(image_id);
		} else {
			throw Error(`'${scanning_mode}' is not a valid scanning mode type`);
		}
	}

	static file_name(image_id) {}

	update(centroid_results) {
		if (this.paused) {
			return;
		}
		...
		if (centroid_results.is_led_on) {
			this.electron_count_on += total_count;
			this.frames_on++;
		} else {
			this.electron_count_off += total_count;
			this.frames_off++;
		}
	}
	...
	get_electrons(count_type, full) {
		let electron_count;
		switch (count_type) {
			case Count_Type.OFF:
				electron_count = this.electron_count_off;
				break;
			case Count_Type.ON:
				electron_count = this.electron_count_on;
				break;
			case Count_Type.TOTAL:
				electron_count = this.electron_count_off + this.electron_count_on;
				break;
			default:
				throw Error(`'${count_type}' is not a valid electron count type`);
				break;
		}
		if (electron_count < 10000 || full) {
			// Return the number unformatted
			return electron_count.toString();
		} else {
			// Return formatted string
			return electron_count.toExponential(3);
		}
	}

	get_frames(count_type, full) {
		let frame_count;
		switch (count_type) {
			case Count_Type.OFF:
				frame_count = this.frames_off;
				break;
			case Count_Type.ON:
				frame_count = this.frames_on;
				break;
			case Count_Type.TOTAL:
				frame_count = this.frames_off + this.frames_on;
				break;
			default:
				throw Error(`'${count_type}' is not a valid frame count type`);
				break;
		}
		if (frame_count < 1000 || full) {
			// Return the number unformatted
			return frame_count.toString();
		} else {
			// Return formatted string
			return Math.floor(frame_count / 1000) + "k";
		}
	}

	// To get file name even if there are no current images (e.g. for file name display)
	static file_name(id) {
		let id_str = ("0" + id).slice(-2);
		return `${Image.formatted_date}i${id_str}.i0N`;
	}

	static ir_file_name(id) {
		let id_str = ("0" + id).slice(-2);
		return `${Image.formatted_date}i${id_str}_IR.i0N`;
	}
	...
	get formatted_date() {
		return Image.formatted_date;
	}
}

class SEVIImage extends Image {
	constructor(image_id) {
		super(image_id);
		this.scanning_mode = Scanning_Mode.SEVI;
	}

	/** Save (combined) VMI image to file */
	save() {
		messenger.add("Saving SEVI scan!");
		let save_path = path.join(settings.save_directory.full_dir, this.file_name);
		// Add IR Off and IR On images together as a string
		let bin_size = settings.centroid.bin_size;
		let image_str = "";
		let pix_value;
		for (let Y = 0; Y < bin_size; Y++) {
			for (let X = 0; X < bin_size; X++) {
				pix_value = Images.images.ir_off[Y][X];
				pix_value += Images.images.ir_on[Y][X];
				image_str += pix_value.toString() + " ";
			}
			image_str += "\n";
		}
		// Write image to file
		fs.writeFile(save_path, image_str, (error) => {
			if (error) {
				messenger.add(`Couldn't save image ${this.id_str}: ${error}`);
			} else {
				messenger.add(`Image ${this.id_str} saved to '${save_path}'`);
			}
		});
	}

	check_autostop() {
		switch (Images.autostop.type) {
			case Autostop.NONE:
				return false;
			case Autostop.ELECTRONS:
				if (this.electron_count_off + this.electron_count_on >= Images.autostop.electrons * 1e5) {
					return true;
				}
				return false;
			case Autostop.FRAMES:
				if (this.frames_off + this.frames_on >= Images.autostop.frames * 1000) {
					return true;
				}
				return false;
			default:
...
class IRImage extends Image {
	...
	/** Save IR Off and IR On images to file */
	save() {
		messenger.add("Saving IR-SEVI scan!");
		let ir_off_save_path = path.join(settings.save_directory.full_dir, this.file_name);
		let ir_on_save_path = path.join(settings.save_directory.full_dir, this.ir_file_name);
		// Convert images to strings
		let ir_off_image_str = Images.images.ir_off.map((row) => row.join(" ")).join("\n");
		let ir_on_image_str = Images.images.ir_on.map((row) => row.join(" ")).join("\n");
		// Write IR Off image to file
		fs.writeFile(ir_off_save_path, ir_off_image_str, (error) => {
			if (error) {
				messenger.add(`Couldn't save IR Off image ${this.id_str}: ${error}`);
			} else {
				messenger.add(`IR Off image ${this.id_str} saved to '${ir_off_save_path}'`);
			}
			console.log("Image saved");
		});
		// Write IR On image to file
		fs.writeFile(ir_on_save_path, ir_on_image_str, (error) => {
			if (error) {
				messenger.add(`Couldn't save IR On image ${this.id_str}: ${error}`);
			} else {
				messenger.add(`IR On image ${this.id_str} saved to '${ir_on_save_path}'`);
			}
		});
	}
	...
	check_autostop() {
		switch (Images.autostop.type) {
			case Autostop.NONE:
				return false;
			case Autostop.ELECTRONS:
				if (this.electron_count_off >= Images.autostop.electrons * 1e5 && this.electron_count_on >= Images.autostop.electrons * 1e5) {
					return true;
				}
				return false;
			case Autostop.FRAMES:
				if (this.frames_off >= Images.autostop.frames * 1000 && this.frames_on >= Images.autostop.frames * 1000) {
					return true;
				}
				return false;
			default:
...
// Need to add functions to update & reset image, save image to file, etc

const Images = {
	all: {}, // Images are stored in a dict with the image_id as the key, that way order doesn't matter
	current: undefined,
	latest: undefined,
	images: {
		id: 0,
	...
	new: (image_id, scanning_mode) => {
		...
		if (Images.all[image_id] && Images.all[image_id].scanning_mode === scanning_mode) {
			new_image = Images.all[image_id];
		} else {
			// Create new image
			new_image = Image.new(image_id, scanning_mode);
			Images.all[image_id] = new_image;
		}
		Images.update_current(new_image);
		return new_image;
	},
	get: (image_id) => {
		return Images.all[image_id];
	},
	update_current: (image) => {
		Images.current = image;
		Images.images.id = image.id;
	},
	clear_current: () => {
		Images.current = undefined;
	},
	update_latest: (image) => {
		Images.latest = image;
	},
	clear_latest: () => {
		Images.latest = undefined;
	},
	reset: () => {
		let bin_size = settings.centroid.bin_size;
		Images.images.ir_off = Array.from(Array(bin_size), () => new Array(bin_size).fill(0));
		Images.images.ir_on = Array.from(Array(bin_size), () => new Array(bin_size).fill(0));
		Images.images.difference = Array.from(Array(bin_size), () => new Array(bin_size).fill(0));
		if (Images.current) {
			Images.current.reset();
		}
		messenger.add("VMI Images reset!");
	},
	save: () => {
		let image = Images.all[Images.images.id];
		// Make sure image exists
		if (image) {
			// Save to file
			image.save();
		} else {
			messenger.add(`Could not find image ${Images.images.id}`);
		}
	},
	update: (centroid_results) => {
		let image_to_update; // Will be either ir_off or ir_on
		let difference_image = Images.images.difference;
		let difference_increment; // Will be +1 for ir_on and -1 for ir_off
		let X, Y; // Filled with centroid values
		// If a scan is not running (or paused), don't update
		if (!Images.current || Images.current.paused) {
			return;
		}
		// Update current scan
		Images.current.update(centroid_results);
		// Update to ir_off or ir_on based on IR LED
		if (centroid_results.is_led_on) {
			image_to_update = Images.images.ir_on;
			difference_increment = 1;
		} else {
			image_to_update = Images.images.ir_off;
			difference_increment = -1;
		}
		...
		for (let i = 0; i < centroid_results.ccl_centers.length; i++) {
			...
			if (centroid_results.ccl_centers[i][0] !== 0) {
				...
				X = Math.round((X * settings.centroid.bin_size) / settings.camera.x_AoI);
				Y = Math.round((Y * settings.centroid.bin_size) / settings.camera.y_AoI);
				image_to_update[Y][X]++;
	...
	update_autostop: (value) => {
		switch (Images.autostop.type) {
			case Autostop.ELECTRONS:
				Images.autostop.electrons = value;
				break;
			case Autostop.FRAMES:
				Images.autostop.frames = value;
				break;
		}
	},
	check_autostop: () => {
...
// Future messages to implement
//		starting melexir... 		finished melexir
//		measuring wavelength... 	wavelength measured
//		single shot image taken		single shot image saved
...
const messenger = {
	...
	add: (message) => {
		if (typeof message === "string") {
			messenger.queue.push(message);
		}
	},
	display: async () => {
		let display_time = 3000;
		let blank_time = 500;
		const message_display = document.getElementById("MessageDisplay");
		if (messenger.fast_display) {
			display_time = 1000;
			blank_time = 250;
		}
		//messenger.flash(); // Show flicker to draw attention to message
		let msg = messenger.queue[0];
		message_display.textContent = msg;
		await sleep(display_time);
		// Remove message from queue and clear display
		messenger.queue.shift();
		message_display.textContent = "";
		await sleep(blank_time);
	},
	display_loop: async () => {
		messenger.active = true;
		while (messenger.active) {
			while (messenger.queue.length > 0) {
				if (messenger.queue.length >= 5) {
					messenger.fast_display = true;
				}
				await messenger.display();
			}
			messenger.fast_display = false;
			// If no messages in queue, wait a bit and check again
			await sleep(250);
		}
	},
	flash: async () => {
		const message_bar = document.getElementById("MsgBar");
		message_bar.style.background = "hsl(225, 20%, 40%)";
		await sleep(100);
		message_bar.style.background = "hsl(225, 20%, 35%)";
	},
	cancel: () => {
