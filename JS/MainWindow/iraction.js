...
actionEmitter.on(IRACTION.UPDATE.OPTIONS, (action_options) => {
	...
	if (ActionManager.status.running) {
		...
		let image_amount_info = {
			image_id: ActionManager.current_image.image_id,
			image_id_str: ActionManager.current_image.image_id_str,
			image_number: completed_image_length + 1,
...
function initialize_action_queue() {
	...
	if (ActionManager.current_image) {
		// Options were updated during a scan - only add images that come after current image
		initial_energy = ActionManager.current_image.expected_excitation_energy.energy.wavenumber;
		step_image_number = ActionManager.current_image.step_image_number + 1;
	} else {
		initial_energy = ActionManager.params.initial_energy;
	}
...
async function run_action_scan() {
	...
	// Need to be able to deal with GoTo or Image being aborted -> use AbortController
	let abort_controller = new AbortController();
	// Define listener functions (to listen for GoTo Cancel or Image Cancel) that we can remove after the scan is finished
	function cancel_function() {
		abort_controller.abort();
	}
	// Set up event listeners
	laserEmitter.on(LASER.ALERT.GOTO.CANCELED, cancel_function);
	seviEmitter.on(SEVI.ALERT.SCAN.CANCELED, cancel_function);
	actionEmitter.on(IRACTION.SCAN.CANCEL, cancel_function);
	...
	while (ActionManager.image_queue.length > 0) {
		...
		uiEmitter.once(UI.RESPONSE.IMAGEID, (id) => {
			...
			let image_amount_info = {
				image_id: id,
				image_id_str: current_image.image_id_str,
				image_number: completed_image_length + 1,
				total_image_number: completed_image_length + image_queue_length + 1, // +1 to account for this image
			};
			actionEmitter.emit(IRACTION.RESPONSE.IMAGEAMOUNT, image_amount_info);
		...
		if (move_wavelength) {
			...
			let current_wavelength;
			try {
				// Reset abort controller (in case it was previously aborted)
				abort_controller = new AbortController();
				laserEmitter.emit(LASER.GOTO.EXCITATION, desired_energy);
				[current_wavelength] = await once(laserEmitter, LASER.RESPONSE.EXCITATION.INFO, { signal: abort_controller.signal });
			} catch (error) {
				// GoTo was canceled
				console.log("Action scan stopped or canceled! (need to add stuff here?)");
				break;
			}
		...
		try {
			...
			await once(seviEmitter, SEVI.ALERT.SCAN.STOPPED, { signal: abort_controller.signal });
		} catch (error) {
			// SEVI scan was canceled
			console.log("Action scan stopped or canceled! (need to add stuff here?)");
			break;
		}
		...
		ActionManager.current_image = undefined;
	}

	ActionManager.duration.end();
	ActionManager.status.running = false;
	ActionManager.status.paused = false;
	ActionManager.status.cancel = false;
	actionEmitter.emit(IRACTION.ALERT.SCAN.STOPPED);
	actionEmitter.emit(IRACTION.RESPONSE.DURATION, ActionManager.duration.convert());

	// Delete all cancel event listeners
	laserEmitter.removeListener(LASER.ALERT.GOTO.CANCELED, cancel_function);
	seviEmitter.removeListener(SEVI.ALERT.SCAN.CANCELED, cancel_function);
	actionEmitter.removeListener(IRACTION.SCAN.CANCEL, cancel_function);
}
